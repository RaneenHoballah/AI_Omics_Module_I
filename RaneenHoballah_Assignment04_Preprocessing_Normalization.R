...
png("PCA_Normalized_Expression.png", width = 800, height = 600)
plot(pca$x[,1], pca$x[,2], col = as.numeric(pdata$group),
     pch = 19, xlab = "PC1", ylab = "PC2",
     main = "PCA of Normalized Expression Data")
legend("topright", legend = levels(pdata$group),
       col = 1:length(levels(pdata$group)), pch = 19)
dev.off()

cat("✅ PCA plot saved as 'PCA_Normalized_Expression.png'\n")
